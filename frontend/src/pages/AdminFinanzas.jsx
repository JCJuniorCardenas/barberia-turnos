...
export function AdminFinanzas() {
  ...
  return <section className="admin-content">
    <p className="section-kicker">Administración</p><h1 className="finance-page-title">Registro financiero</h1>
    {error && <p className="error-message" role="alert">{error}</p>}{message && <p className="success-message" role="status">{message}</p>}
    <div className="finance-summary-cards"><article className="finance-summary-card"><span>Ingresos</span><strong className="finance-income">${Number(balance.ingresos).toLocaleString('es-AR')}</strong></article><article className="finance-summary-card"><span>Egresos</span><strong className="finance-expense">${Number(balance.egresos).toLocaleString('es-AR')}</strong></article><article className="finance-summary-card"><span>Balance neto</span><strong className="finance-balance">${Number(balance.balance).toLocaleString('es-AR')}</strong></article></div>
